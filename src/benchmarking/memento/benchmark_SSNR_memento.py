...
def main(raw_dataset_path, processed_dataset_path):
    name = sys.argv[1]
    names = sorted([f for f in os.listdir(raw_dataset_path) if ((f.endswith("hdf5") or f.endswith("h5ad")) and (name in f))]) #  ]
    files = [os.path.join(raw_dataset_path, f) for f in names]
    
    print("DATASET FILES", files)
    
    for f in files:
        basen = os.path.basename(f)
        p = f"/home/woody/iwbn/iwbn007h/scXMatch_paper/evaluation_results/1_10_SSNR_memento/memento_benchmark_results_{basen}_with_replicates.csv"
        if os.path.exists(p):
        ...
        for i in datasets:
            ...
            if "bhatta" in name:
                processed_p = os.path.join(processed_dataset_path, f"processed_{basen.split('.')[0]}_{bhattacherjee_idxs[i]}.hdf5")

            processed_adata = ad.read_h5ad(processed_p)

            groups = current_dataset.obs[group_by].unique()
            for test_group in groups:
                if test_group == reference:
                    continue    
                subset = current_dataset[current_dataset.obs[group_by].isin([test_group]), :].copy()
                ...
                for group_by_split in ["split_50", "split_10", "split_30"]:
                    ...
                    memento_log_df["dataset"] = os.path.basename(processed_p).split(".")[0]
                    all_results.append(memento_log_df)
        all_results_df = pd.concat(all_results)
        all_results_df.to_csv(p)
            
        
if __name__ == "__main__":
    main(raw_dataset_path="/home/woody/iwbn/iwbn007h/data/scrnaseq_ji/raw", processed_dataset_path="/home/woody/iwbn/iwbn007h/data/scrnaseq_ji/")