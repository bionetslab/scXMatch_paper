...
def main(dataset_path):
    ...
    for f in files:
        ...
        p = f"/home/woody/iwbn/iwbn007h/scXMatch_paper/evaluation_results/1_4_edist/no_subsampling_edist_benchmark_results_{basen}_{subset}_with_20k.csv"
        if os.path.exists(p):
        ...
        if name == "schiebinger":
            assert ((subset == 0) or (subset == 1) or (subset == 2) or (subset == 3) or (subset == 4) or (subset == 5))
            if subset == 0:
                ...
                subset = adata[adata.obs[group_by].isin(["control", 'D8', 'D2', 'D4', 'D6', 'D3.5'])]
            elif subset == 5:
                subset = adata[adata.obs[group_by].isin(["control", 'D5', 'D4.5', 'D2.5', 'D3', 'D5.5'])]
